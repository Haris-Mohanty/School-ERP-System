    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if(response.trim() == "success"){
          ...
          window.location = "php/invoice.php?enrollment="+allInput[1].value+"&name="+allInput[2].value+"&category="+allInput[3].value+"&date="+finalDate+"&course="+allInput[4].value+"&batch="+allInput[5].value+"&fee-time="+allInput[6].value+"&paid-fee="+paid_fee+"&pending="+fee_pending+"&recent="+allInput[8].value;
          invoiceForm.reset();
        }else{
          swal(response.trim(), response.trim(), "error");
        }
